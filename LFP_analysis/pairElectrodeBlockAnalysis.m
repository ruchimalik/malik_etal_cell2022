function [allPower, allCovar, allCovarPhase, allPLS, allPlsPhase, allWpli, ...
    allCoh,allPli, allPSI, expLogPower, expCovar, expCovarPhase, expPLS, expPlsPhase, expWpli, ...
    expCoh,expPli,expPSI, allPowerTypes, allPowerExps, allPowerTreatments, allPairTypes, allPairExps, allPairTreatments] = ...
    pairElectrodeBlockAnalysisMC(filelist, fileprefix, channelIDs, types, treatments, animals, electrodeIDs)
...
%     electrodeIDs = {'Hpc';'Pfc'};
    ...
    allPli = cell(nelpairs, nbands);
    allPSI = cell(nelpairs, nbands);
        ...
        parfor elpairIDX=1:nelpairs,
            for bandIDX=1:nbands,
                for typeIDX=1:ntypes,
                    ...
                    allPli{elpairIDX,bandIDX} = [allPli{elpairIDX,bandIDX}, trialPli{elpairIDX,typeIDX,bandIDX}];
                    allPSI{elpairIDX,bandIDX} = [allPSI{elpairIDX,bandIDX}, trialPSI{elpairIDX,typeIDX,bandIDX}];
...
%     
%     expPli = [animalArray typeArray treatmentArray squeeze(expPli)];
%     csvwrite(strcat(fileprefix,'avgPLI.csv'), expPli);
%     
%     expPSI = [animalArray typeArray treatmentArray squeeze(expPSI)];
%     csvwrite(strcat(fileprefix,'avgPSI.csv'), expPSI);
%     
...
csvwrite(strcat(fileprefix,'avg',electrodeIDs{2},'Power.csv'), el2Pwr);
 ...
 allWPLI_fin = cell2mat(allWpli');
 allWPLI = [allPairTypes{1,1}' allWPLI_fin'];
csvwrite(strcat(fileprefix,'allWPLI.csv'), allWPLI);
 
allPOWER_elec1 = (allPower(1,:));
allPOWER_elec1 = cell2mat(allPOWER_elec1');
el1_allPwr = [allPowerTypes{1,1}' allPOWER_elec1'];
csvwrite(strcat(fileprefix,'all',electrodeIDs{1},'power.csv'), el1_allPwr);

allPOWER_elec2 = (allPower(2,:));
allPOWER_elec2 = cell2mat(allPOWER_elec2');
el2_allPwr = [allPowerTypes{1,1}' allPOWER_elec2'];
 csvwrite(strcat(fileprefix,'all',electrodeIDs{2},'power.csv'), el2_allPwr);
