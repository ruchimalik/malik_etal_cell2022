...
% $$$     end
 ...
 % do it channel-by-channel to save memory
  for k = nchans:-1:1 % reverse loop so that first iteration preallocates
    ...
    % interp1 can barf on single-precision data, so cast to double if
    % needed.
    cdatatype = class(c.data);
    if ~isempty(a.extrapval)
      newdata(:,k) = cast(interp1(x,double(c.data(:,k)), xi, a.method, a.extrapval), cdatatype);
    else
      newdata(:,k) = cast(interp1(x,double(c.data(:,k)), xi, a.method), cdatatype);
    end
  end
