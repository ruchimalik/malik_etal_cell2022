...
if a.recover_carriers_from_signal, 
    ...
    for j = 1:a.nsignals;
        ...
        [PSDs.Pxx_Ref(:,j), PSDs.F_Ref(:,j)] = ...
            periodogram(detrend(c_Raw.data(:,chanidx),'constant'),blackman(size(c_temp.data,1)),[],c_Raw.samplerate);

        % when auto-detecting Ref frequency, ignore peaks near DC/below this frequency (Hz)
        F_ref_min = 1; 
        F_trunc_idx = find(PSDs.F_Ref(:,j)>1, F_ref_min, 'first');
        Ref_trunc = PSDs.F_Ref(F_trunc_idx:end,j);
        Pxx_trunc = PSDs.Pxx_Ref(F_trunc_idx:end,j);

        [~,maxidx] = max(Pxx_trunc);
        Ref_F(j) = double(Ref_trunc(maxidx)); % needs to be double since it refers to time
    end
    ...
    if any(synthY),
        for j = 1:a.nsignals,
            if synthY(j),
                ...
                delay = 1/4 * 1/Ref_F(j); %<-- this is dividing entire time by 4
                % delay = 0;
