...
Args = struct('Axis', [], 'XLim', [], 'YOffset', [], 'Height', [], 'FaceColor', [], 'Alpha', 0.2, 'EdgeColor', 'none', 'LineWidth', 2, 'LineStyle', '-', 'SegNames', {''}, 'TextOffset', 0, 'PlotArea', 1, 'ShowText', 0, 'TextColor', []);
...
if (nargin>1)
    try
        Args = parseArgsFK(varargin, Args);
    catch
    ...
    if Args.PlotArea
        for c=1:nsegments
          ...
          if isempty( segments{c} )
            ...
            if length(id)>0
                ptch = patch([segments{c}(id,1) segments{c}(id,1) segments{c}(id,2) segments{c}(id,2)]', repmat([Args.YOffset(c) Args.YOffset(c)+Args.Height(c) Args.YOffset(c)+Args.Height(c) Args.YOffset(c)]', 1, length(id)), [0 0 0], 'FaceColor', Args.FaceColor(c,:), 'EdgeColor', Args.EdgeColor(c,:), 'HitTest', 'off');
