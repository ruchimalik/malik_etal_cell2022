function [r pks tc clh cls_ranked clcolors] = mkraster_multi(varargin)
% MKRASTER make a spike raster plot image
%
% $Id: mkraster.m 476 2008-04-04 18:17:52Z tjd $
  ...
  if isempty(r.clnos),
    r.clnos = 1:length(r.e.epoch(r.epochi).cl);
  end
  ...
  switch r.rasteropt.colormode
   ...
   case 'electrode'
    ...
    for j = 1:length(r.e.electrode);
      clcolors(r.e.electrode(j).cls{r.epochi}) = ...
          mod(j,nclcolors) + 1;
    end
