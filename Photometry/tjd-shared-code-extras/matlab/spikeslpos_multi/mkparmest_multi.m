function [pe tc clh] = mkparmest_multi(varargin)
...
%   -test that we have all the inputs
  ...
  if isempty(pe.clnos),
    pe.clnos = 1:length(pe.e.epoch(pe.epochi).cl);
  end
  ...
  if isempty(pe.clnos),
    clnos = 1:length(pe.e.epoch(pe.epochi).cl);
  else
    clnos = pe.clnos;
  end
  ...
  if isempty(tc)
    ...
    tc = mktunecurve_multi('e',pe.e,...
                     'clnos',clnos,...
                     'tunecurveopt', pe.tunecurveopt,...
                     'cache', pe.cache);

    % select appropriate tuning curves for e.epoch(epi).cl(clnos)
...
% $$$   end
  ...
  pos_t = getepd(pe.e.epoch(pe.epochi).pos,'time');
  ...
  try
    pe.parm = getepdi(pe.e.epoch(pe.epochi).pos, pe.posi, pe.tunecurveopt.tuneparams{:});
  catch
    pe.parm = zeros(size(pe.posi));
    warning('no matching tuneparams--returning 0s'); 
  end
