function [sla sld slcache arglist] = spikeslpos_multi (varargin)
...
%
% *       'epoch': name (if char array) /index (if numeric) of epoch to display
%
...
%
%      'runspeed': runspeed filter for place field finding (10 cm/s)
%      'runepoch': which epoch to use for tunecurves ([] = use
%                  first run epoch)
%
...
% _epoch_
% get the epoch to plot (note currently spikeslpos can only handle a
% single epoch at a time)

if ischar(a.epoch),
  epi = find(strcmp(a.epoch, {e.epoch.name}));
  switch numel(epi)
   case 0
    error(['requested epoch name not found in exp struct: ' a.epoch]);
   case 1
    %ok
   otherwise
    error(['Multiple epochs match requested epoch name: ' a.epoch]);
  end
elseif isnumeric(a.epoch)
  epi = a.epoch;
  if numel(epi)~=1,
    error('Exactly one epoch must be specified');
  end
  if epi<1 || epi>numel(e.epoch),
    error(['Requested numeric epoch index ( ' num2str(a.epoch) ' ) out ' ...
           'of range (1 - ' num2str(numel(e.epoch)) ').'])
  end
else
  error(['''epoch'' must be either a string containing the name of a ' ...
         'single epoch, or a numeric index into e.epoch.']);
end
...
% _runepochi_
if isempty(a.runepoch)
  % default is current epoch (if behav==run), or first run epoch.
  if strcmp('run', e.epoch(epi).behav)
    run_epi = epi;
  else
    run_epi = find(strcmp('run', {e.epoch.behav}), 1,  'first');
    warning(['no runepoch specified, using first ''run'' epoch found: #' ...
             num2str(run_epi)]); %#ok
  end
else
  if ischar(a.runepoch),
    run_epi = find(strcmp(a.runepoch, {e.epoch.name}));
    switch numel(run_epi)
     case 0
      error(['requested runepoch name not found in exp struct: ' a.epoch]);
     case 1
      %ok
     otherwise
      error(['Multiple epochs match requested runepoch name: ' a.epoch]);
    end
  elseif isnumeric(a.runepoch)
    run_epi = a.runepoch;
    if numel(run_epi)~=1, % empty array or multiple epis
      error('No more than 1 runepoch may be specified');
    end
    if run_epi<1 || run_epi>numel(e.epoch),
      error(['Requested numeric runepoch index ( ' num2str(a.runepoch) ' ) out ' ...
             'of range (1 - ' num2str(numel(e.epoch)) ').'])
    end
  else
    error(['''runepoch'' must be either a string containing the name of a ' ...
           'single epoch, or a numeric index into e.epoch.']);
  end
end
...
% _plots_
if isempty(a.plots)
  sp = plotdefs_multi('e', e, 'epochi', epi, 'runepochi', run_epi);
  a.plots = sp.default;
end
...
set(figp,'keypressfcn', 'slkeypress_multi(gcbf)');
set(figp,'windowbuttondownfcn', 'slmousedown_multi(gcbf)');
set(figp,'windowbuttonmotionfcn', 'slmousemove_multi(gcbf)');
set(figp,'windowbuttonupfcn', 'slmouseup_multi(gcbf)');
set(figp,'interruptible','off');
set(figp,'resizefcn','slresize_multi(gcbf)');
...
if isstruct(sld) && ...
  ...
  for j = clnos
    cltsj = getepd(e.epoch(epi).cl(j), 'time');
    if cltsj
...
pos_timei = getepindex(e.epoch(epi).pos, 'time');
if ~isempty(pos_timei)
    mints = min([mints e.epoch(epi).pos.dat(1,pos_timei)]);
    maxts = max([maxts e.epoch(epi).pos.dat(end,pos_timei)]);
end
...
for k = 1:nplots;
  ...
  % set epochi struct name of all plots
  if isfield(a.plots{k}.dat, 'epochi'),
    a.plots{k}.dat.epochi = epi;
  end

  % set clnos of all plots
  if isfield(a.plots{k}.dat, 'clnos') && ~isempty(clnos)
    a.plots{k}.dat.clnos = clnos;
    a.plots{k}.dat.clperm = clperm;
  end
  
  % set runepoch of all plots
  if isfield(a.plots{k}, 'tunecurveopt')
    a.plots{k}.tunecurveopt.runepochi = run_epi;
  end
