...
% choosen, detrend, ...?
  ...
  if a.segdata_only,
    ...
    [dum nonsegsi] = contsegdata(a.cdat, seg_not(a.segs, ...
  ...
  switch(a.trigtype)
      ...
      case 'event',
          ...
          [dummy nev_per_seg nth_ev_per_seg] = inseg(p.segs,a.events_t);
...
% $$$   end
  ...
  p.peritrig_std = nanstd(p.peritrig_data,1); %  p.peritrig_std = nanstd(p.peritrig_data,0,1)
  ...
  switch a.plottype
   case 'trigave',
    ...
    plusminus = [];
    if a.plotci,
      plusminus = p.peritrig_sem; %deleted 1.96* before p.peritrig_sem
    elseif a.plotsem,
...
%       else
    ...
    if a.title,
      title(ax, ['Triggered average of ' dat_name '; n = ' ...
                 num2str(size(p.peritrig_data,1)) sprintf('\n') ,...
