...
%
% output 'filt' structure with fields:
%  name: theta, e.g.
%  Fs: sampling frequency
%  filttype: 'high'/'low'/'band'
%  F: design frequencies
%  atten_db/ripp_db: arguments
%  dfilt: discrete filter object we just designed
...
switch filt.filtopt.filttype
 ...
 case 'dcblock'
  if numel(filt.filtopt.F)==1,
     p = dcblock(filt.filtopt.F(1),filt.filtopt.Fs);  % get filter coefficient
     fB = [1 -1];                         % set up differentiator
     fA = [1 -p];        % set up integrator
  end
